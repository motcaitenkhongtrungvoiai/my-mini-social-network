    ...
    checkBTN = !checkBTN; 
        ...
        async function loadUsers() {
            try {
                ...
                data.users.forEach((user) => {
                    ...
                    tr.innerHTML = `
                        <td><img src="${user.avatar}" width="40" height="40" class="avatar" onerror="this.src='http://localhost:3000/access/default.png'"/></td>
                        <td>${user.username}</td>
                        <td>${user.email}</td>
                        ...
                        <td>
                            <select id="select-${user._id}">
                                ...
                                <option value="criminal" ${user.role === "criminal" ? "selected" : ""}>criminal</option>
                            </select>
                            <button class="btnChange" data-userid="${user._id}">Update</button>
                            ${user.reportCount > 0 ? `<button class="btnBan" data-userid="${user._id}">Ban</button>` : ''}
                        </td>
                        <td><a href="../../public/profile.html?data=${user._id}" target="_blank">View Profile</a></td>
                    `;
        ...
        async function changeRole(userId, isReportedTable = false) {
            try {
                ...
                const newRole = document.getElementById(`${selectorPrefix}select-${userId}`).value;
                ...
                document.getElementById(`${selectorPrefix}role-${userId}`).innerText = newRole;
                document.getElementById(`${selectorPrefix}select-${userId}`).value = newRole;
                ...
                if (isReportedTable) {
                    ...
                    const reportedRoleElement = document.getElementById(`reported-role-${userId}`);
                ...
                const row = document.getElementById(`${selectorPrefix}select-${userId}`).closest("tr");
                ...
                if (newRole === "criminal" || newRole === "user") {
                    const criminalCount = Array.from(document.querySelectorAll("td[id^='role-'], td[id^='reported-role-']"))
